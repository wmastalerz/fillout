...
INTER_CELL_CS_HO_SUC (unsigned int)";M.Skibinski;22D;Counter
...
";M.Skibinski;22D;
GSM ANR;GSM-SO0103;GSM Inter-RAT ANR for WCDMA;GSM-SO0103 - GSM Inter-RAT ANR for WCDMA;FR110;110;"[C_BSC] BSS shall support statistics from handover counters for ANR 3G NCR ranking and deleting. The counter family display name is ""inter RATUTRANHandover outgoing Target"" which is developed in GSM-SW2029 feature.";M.Skibinski;22D;Counter
...
";M.Skibinski;22D;
GSM ANR;GSM-SO0104;GSM Inter-RAT ANR for LTE;GSM-SO0104 - GSM Inter-RAT ANR for LTE;FR110;110;"[C_BSC] BSS shall support statistics for ANR 4G NCR ranking and deleting. The counter type name is ""BS_FAST_RETURN_SUC_TO_EUTRAN"" which is developed in GSM-SW0305 feature.";M.Skibinski;22D;Counter
...
GSM ANR;GSM-SO0102;Intra-GSM ANR;GSM-SO0102 - Intra-GSM ANR;;;;M.Skibinski;00-OLD-SON;
GSM ANR;GSM-SO0301;2G-4G Automatic Frequency Planning;GSM-SO0301 - 2G-4G Automatic Frequency Planning;;;;M.Skibinski;00-OLD-SON;
GSM ANR;GSM-SO0103;GSM Inter-RAT ANR for WCDMA;GSM-SO0103 - GSM Inter-RAT ANR for WCDMA;;;;M.Skibinski;00-OLD-SON;
GSM ANR;GSM-SO0104;GSM Inter-RAT ANR for LTE;GSM-SO0104 - GSM Inter-RAT ANR for LTE;;;;M.Skibinski;00-OLD-SON;
...
ANR;FGS-SF0103;ANR;FGS-SF0103-ANR;FR70;70;"SON Server(VNF)? ???? NR ?? ?? ??? ?? Functional Switch, Schedule ? ???? ?? ??? ????? ??.
   - Schedule: start time, duration, period, amount
   - ????: Handover ?? threshold (Count, ?? ?), ?? ?? ?

SON Server (VNF) shall be able to set functional switch and configure activation schedule and related parameters for unnecessary NR deletion.
   - Schedule: start time, duration, period, amount
   - Parameters: Handover-related threshold (count, performance, etc.)";R.Rohulko;00-OLD-SON;
ANR;FGS-SF0103;ANR;FGS-SF0103-ANR;FR110;110;"SON Server (VNF)? ?? 3?? ?? ?? ?, Initial NR? ???? 5G EMS (VNF)? ????? ??. ?, NR ?? ??? SON Server ?? ?? ????? ??.
  . Cell creation complete
  . The flag in cell creation profile is set to be TRUE. The flag in cell creation profile means whether the initial NR is generated automatically or not
  . AU's GPS location (Lat./Lon.) infomation is acquired

SON Server (VNF) shall generate and transfer the initial NR(s) to 5G EMS (VNF), when satisfing the following conditions. At this time, the NR creation subject should be set as ANR_SON Server and transfered to 5G EMS together.
  . Cell creation complete
  . The flag in cell creation profile is set to be TRUE. The flag in cell creation profile means whether the initial NR is generated automatically or not
  . AU's GPS location (Lat./Lon.) information is acquired";R.Rohulko;00-OLD-SON;
ANR;FGS-SF0103;ANR;FGS-SF0103-ANR;FR120;120;"SON Server (VNF)? Cell Creation Profile?? ???? ?? initial parameter ? ?? ?, ?? cell? Cell Creation Profile? ???? ??.
  - Initial parameters: PCI, NR, RSI, etc.

After SON Server (VNF) allocates all initial parameters requested in cell creation profile, SON Server (VNF) shall detete the cell creation profile.
  - Initial parameters: PCI, NR, RSI, etc.";R.Rohulko;00-OLD-SON;
ANR;FGS-SF0103;ANR;FGS-SF0103-ANR;FR150;150;"5G-NB? UE? ?? ??? ECGI? ???? cell? ?? 5G-NB? NR-X2 TNL Address? ??? ??, 5G EMS(VNF)? ???? SON Server (VNF)?? NR-X2 TNL Address? ????.

The 5G-NB shall request the NR-X2 TNL address of the new 5G-NB to the SON Server (VNF) via the 5G EMS (VNF) if the 5G-NB does not know the NR-X2 TNL address of the new 5G-NB which includes the cell having the same ECGI as the ECGI obtained from UE.";R.Rohulko;00-OLD-SON;
ANR;FGS-SF0103;ANR;FGS-SF0103-ANR;FR160;160;"SON Server(VNF)? 5G EMS(VNF)? ??? 5G-NB? ?? ?? NR? ECGI ??? ??? ??, ?? ECGI? ?? cell ? ?? 5G-NB? NR-X2 TNL Address? ???? 5G EMS(VNF)? ???? 5G-NB? ????.

SON Server (VNF) shall retrieve the NR-X2 TNL address of the 5G-NB to which the cell having the ECGI belongs, and transfer the NR-X2 TNL address to the 5G-NB via 5G EMS (VNF).";R.Rohulko;00-OLD-SON;
ANR;FGS-SF0103;ANR;FGS-SF0103-ANR;FR180;180;"SON Server(VNF)? Initial NR ?? ?, Co-Site AU? ?? 5G-NB? ??, ??? NR ?? ??? ????? ??.

SON Server (VNF) shall provide bi-directional NR generation function when initial NR includes the cells whose AU is co-site and belongs to the same 5G-NB.";R.Rohulko;00-OLD-SON;
ANR;FGS-SF0103;ANR;FGS-SF0103-ANR;FR190;190;"5G-NB? ?? NR ?? ?, ?? ??? ?? NR ?? ??? ?? ????? ??.
   - ?? ??: Operator, ANR_RAN, ANR_SON Server
   - ?? ??: YY-MM-DD HH-MM-SS

When creating a new NR, 5G-NB shall record the subject and NR creation time together.
   - Created by: Operator, ANR_RAN, ANR_SON Server
   - Creation time: YY-MM-DD HH-MM-SS";R.Rohulko;00-OLD-SON;
ANR;FGS-SF0103;ANR;FGS-SF0103-ANR;FR310;310;"SON Server(VNF)? NR ?? ?? ??? ????? ??.
   - NR ?? ?? ??: ?? cell ? ?? (NRT ?? ???? ??)? ??? ??, ?? cell ? NRT ? ???? ?? cell ? NRT ??? ???? ???? ??
  - ?, NR-X2 ? ???? ?? 5G-NB ?? ?? cell ? NRT? ???? ?? ??? ???.
    (?, intra-5GNB ? NR-X2? ??? ??, 5G-NB ?? ???? ??)

SON Server (VNF) shall provide the NR information modification function.
   - NR information modification function: automatically change the NRT information of the cell that contains the cell in the NRT when the information of the specific cell (information managed by NRT) is changed
  - However, this function is limited to the case that a 5G-NB include the corresponding cell in NRT but does not have NR-X2 link.";R.Rohulko;00-OLD-SON;
ANR;FGS-SF0103;ANR;FGS-SF0103-ANR;FR320;320;"SON Server(VNF)? Cell ?? ?, ?? cell ? NRT ? ???? ?? cell? NRT?? ??? cell ? ???? ??? ???? ??.
  - ?, NR-X2 ? ???? ?? 5G-NB ?? ?? cell ? NRT? ???? ?? ??? ???.
    (?, intra-5GNB ? NR-X2? ??? ??, 5G-NB ?? ???? ??)

SON Server (VNF) shall provide the function to remove the cell in NRT when a cell is deleted.
   - For example, cell B includes cell A in NRT. When cell A is deleted, SON Server (VNF) remove the cell A in cell B's NRT.
  - However, this function is limited to the case that a 5G-NB include the corresponding cell in NRT but does not have NR-X2 link.";R.Rohulko;00-OLD-SON;
ANR;FGS-SF0103;ANR;FGS-SF0103-ANR;FR330;330;"SON Server(VNF)? ????? Handover ?? ???? ???? NR? ???? ??? ????? ??.
   - ???? NR: Handover Count ? ??? ??, Handover Count ? ??? ??? ??
  - ?? ?? ? count/?? threshold ? ?? ?? ????? ?. (FR 70 ??)
  . ?, NR ??? is-remove-allowed = False ???, NR ?? ?? ??? ???? ?? ?? ?? ?? ??.

SON Server (VNF) should provide the function to remove unnecessary NRs based on handover statistics periodically
   - Unnecessary NR: Handover count is less than threshold, handover count and performance is less than threshold
   - However, if the NR property is is-remove-allowed = False or the period after NR generation is less than a certain period, it shall be excluded.";R.Rohulko;00-OLD-SON;
ANR;FGS-SF0103;ANR;FGS-SF0103-ANR;FR350;350;"SON Server(VNF)? ????? ???? NR-X2 NR ??? ?? ?? ??? ??, NR-X2 NR? ???? ??? ???? ??.
   - ?, NR-X2 white list? ??? ??? ????.

SON Server (VNF) shall provide the function to remove the NR-X2 NR periodically, after a certain period of time in the unnecessary NR-X2 NR state.
   - Except in the case of inclusion in the NR-X2 white list.
";R.Rohulko;00-OLD-SON;
ANR;FGS-SF0103;ANR;FGS-SF0103-ANR;FR410;410;"SON Server(VNF)? ???? ??? cell ? ?? ?? ??? ?? ?? ???? ?? NR cell ? ??? ?? (Map) ?? ??? ? ??? ??.
   - Cell ?? : ??, ??? ?
   - ??, ? cell ? ? ?? (distance) ?? ? ??? Neighbor Relationship ??? ?? ??? ? ??? ??.
   ? ?? ???? ?? display?? ??? TBD

SON Server (VNF) shall be able to display on the map the information of the NR cells having the same frequency (or all frequencies) as the cell selected by operator
   - Cell information: location, azimuth, etc.
   - At this time, distance information from each cell and bi-directional neighbor relationship shall be displayed together.
    ? The function for all frequencies is TBD";R.Rohulko;00-OLD-SON;
ANR;FGS-SF0103;ANR;FGS-SF0103-ANR;FR420;420;"SON Server(VNF)? ???? ??? Cell Group/??? ??, ??? neighbor relationship? ?? cell ?? ?? ??? ????? ??. ??, ???? ??? neighbor relationship? ?? ??, ??? NR? ?? ??? ????? ??.
   . ??, Cell ??? source cell, target cell ? ECGI (PLMN ID + 5G-NB ID + Cell ID), 5G-ARFCN, PCI ? source cell ? target cell? distance ? ??


SON Server (VNF) shall be able to retrieve the cells having unidirectional neighbor relationship for the cell group / area selected by the operator. In addition, if the operator request a bidirectional neighbor relationship, SON Server (VNF) shall provide a function to establish bi-directional NR.
   . In this case, the cell information includes the source cell, the ECGI (PLMN ID + 5G-NB ID + Cell ID) of the target cell, the 5G-ARFCN,";R.Rohulko;00-OLD-SON;
ANR;FGS-SF0103;ANR;FGS-SF0103-ANR;FR430;430;"SON Server(VNF)? ???? ??? 5G-NB ? ?? NR-X2? ??? 5G-NB? cell ??? ?? (Map) ?? ??? ? ??? ??. [TBD]
   . Cell ?? : ??, ???, sector beamwidth
   . ??, ?? 5G-NB? cell? ?? color, ?? ?? 5G-NB ? cell ? ?? ?? color ? ????.";R.Rohulko;00-OLD-SON;
ANR;FGS-SF0103;ANR;FGS-SF0103-ANR;FR440;440;"SON Server(VNF)? ???? ??? cell? ?? ?? ??? ?? ?? ??? NR cell ?? Handover ?? ??? ?? (Map) ?? ??? ? ??? ??. ??, Handover? Network Controlled Handover? UE Based Handover? ?? ? ??? ? ??? ??.
   - Cell ??: ??, ??? ?
   - Handover ?? ??: Handover ??/?? count, ???
   - Handover ???? color (e.g.: Red, Orange, Yellow, Green)? mapping ?? rule ? ?? ????? ??.
  ? ?? ???? ?? display?? ??? TBD

SON Server (VNF) shall be able to display on the map handover related information between a cell selected by operator and its NR cells having the same frequency (or all frequencies) as the cell. The handover information shall be able to distinguish/separate network controlled handover and UE based handover, and integrate those handovers.
   . Cell information: location, azimuth, etc.
   . Handover information: Handover attempt / success count, success rate etc.
   . The rules for mapping handover success rates to color (eg, Red, Orange, Yellow, Green) shall be configurable.
    ? The function for all frequencies is TBD";R.Rohulko;00-OLD-SON;
ANR;FGS-SF0103;ANR;FGS-SF0103-ANR;FR450;450;"SON Server(VNF)? NR map? ?? ??, ?? ? ?? ??? ????? ??.

SON Server (VNF) shall support the function of zoom-in, zoom-out and movement for the NR map.";R.Rohulko;00-OLD-SON;
ANR;FGS-SF0103;ANR;FGS-SF0103-ANR;FR460;460;"SON Server(VNF)? ?? (Map) ??? ???? ??? 2? cell ? ?? ??? neighbor relationship ? ?? ??? ???? ??.

SON Server (VNF) shall be able to establish a bi-directional neighbor relationship for two cells selected by the operator on the map.
";R.Rohulko;00-OLD-SON;
ANR;FGS-SF0103;ANR;FGS-SF0103-ANR;FR470;470;"SON Server(VNF)? ?? (Map) ??? ???? ??? ? cell? NR?? ??? ???? ??? ????? ??.
   - ?, ???? ??? ?? ?, NR? ????.
   - ?, ????? ??? ??, ??? ????.

SON Server (VNF) shall provide the function to delete/remove the neighbor relation between two cells selected by the operator on the map.
   . However, after the re-verification to the operator, NR shall be deleted/removed.";R.Rohulko;00-OLD-SON;
ANR;FGS-SF0103;ANR;FGS-SF0103-ANR;FR480;480;"SON Server(VNF)? NR ?? ??? file ? ???? ??? ????? ??.

SON Server (VNF) shall the function to save NR analysis results as a file.";R.Rohulko;00-OLD-SON;
...
[EMS][P_AU][P_DU][C_DU][C_UDU][VM_CU][C_CU][non-vRAN][Smallcell][NVGNB_AU]
EMS shall inform gNB that NR NCR addition is not possible due to distance limitation if the distance between source NR cell and target NR cell exceeds a specific threshold informed by gNB when performing UE-based ANR or EMS-based ANR.";R.Rohulko;00-OLD-SON;
Algorithms;FGS-SF0102;PCI Auto-Configuration;FGS-SF0102 - PCI Auto-Configuration;FR20;20;"SON Server(VNF)? ???? PCI ?? ?? ?? ??? ????? ??.
  . PCI ?? ??: Not Need Approval,  Need Approval

SON Server (VNF) shall be able to set whether PCI change application by 'PCI auto-configuration' needs operator's approval or not..
  . PCI change application by 'PCI auto-configuration': Not Need Approval, Need Approval";L.Polek;00-OLD-SON;
Algorithms;FGS-SF0102;PCI Auto-Configuration;FGS-SF0102 - PCI Auto-Configuration;FR30;30;"SON Server (VNF)? ???? PCI ?? time schedule ?? ??? ????? ??. [TBD]

SON Server shall be able to set the schedule for PCI change application by 'PCI auto-configuration'. [TBD]";L.Polek;00-OLD-SON;
...
Algorithms;FGS-SF0102;PCI Auto-Configuration;FGS-SF0102 - PCI Auto-Configuration;FR180;180;"SON Server(VNF)?  Fragmented BW Cell ? ?? PCI ?? ??? ???? ??? ??.

SON shall not perform 'PCI conflict detection' for the cells having fragmented bandwidth.";L.Polek;00-OLD-SON;
Algorithms;FGS-SF0102;PCI Auto-Configuration;FGS-SF0102 - PCI Auto-Configuration;FR190;190;"SON Server(VNF)? ????? ? cell? neighbor cell list ???? PCI ??? ??? ? ??? ??.
  - ?? ??:  PCI Auto-Configuration  ???? ?, 5G-NB ? PCI ?? ?? ??? ??? PCI ?? ??? ???? ??

SON Server (VNF) shall be able to to detect PCI conflict in periodic based on NRT.
  - Background: When 'PCI Auto-Configuration' is de-activated, 5G-NB does not perform PCI conflict detect";L.Polek;00-OLD-SON;
Algorithms;FGS-SF0102;PCI Auto-Configuration;FGS-SF0102 - PCI Auto-Configuration;FR310;310;"SON Server (VNF)? ?? 3?? ?? ?? ?, Initial PCI? ???? 5G EMS (VNF)? ????? ??.
  . Cell creation complete
  ...
  . AU's GPS location (Lat./Lon. Info.) is acquired.

SON Server (VNF) shall select and transfer the initial PCI to 5G EMS (VNF), when satisfing the following conditions.
  . Cell creation complete
  . The flag in cell creation profile is set to be TRUE. The flag in cell creation profile means whether the initial PCI is allocated automatically or not
  . AU's GPS location (Lat./Lon. Info.) is acquired.";L.Polek;00-OLD-SON;
Algorithms;FGS-SF0102;PCI Auto-Configuration;FGS-SF0102 - PCI Auto-Configuration;FR320;320;"SON Server (VNF)? Cell Creation Profile?? ???? ?? initial parameter ? ?? ?, ?? cell? Cell Creation Profile? ???? ??.
  - Initial parameters: PCI, NR, RSI, etc.

After SON Server (VNF) allocates all initial parameters requested in cell creation profile, SON Server (VNF) shall detete the cell creation profile
  - Initial parameters: PCI, NR, RSI, etc.";L.Polek;00-OLD-SON;
Algorithms;FGS-SF0102;PCI Auto-Configuration;FGS-SF0102 - PCI Auto-Configuration;FR330;330;"SON Server(VNF)? 5G EMS(VNF)? ??? 5G-NB ? PCI ?? ?? ?? ? ?? PCI ?? ?? ?, PCI ?? ??? ? cell ??? ?? ??? ???? PCI ? ??? cell ? ????? ??.
   . ECGI (PLMN + CellIdentity )??

SON Server(VNF) shall be able to select the cell whose PCI to be changed base on the following information, when SON Server (VNF) knows/recognizes 'PCI conflict'
   . Recognition of PCI conflict: receive 'PCI conflict report', detect PCI conflict in periodic.
   . Information: ECGI (PLMN + CellIdentity )
";L.Polek;00-OLD-SON;
Algorithms;FGS-SF0102;PCI Auto-Configuration;FGS-SF0102 - PCI Auto-Configuration;FR340;340;"SON Server(VNF)? PCI ??? ?, PCI white list ? ???? PCI ? ????? ??.
   . ??, ??? PCI ? ?? cell ?  To Be PCI  ? ???? ?? cell ?? PCI ???? ????.

SON Server (VNF) shall select a PCI included in PCI white list when re-allocating PCI.
  . The selected PCI shall be managed as 'To Be PCI' of the corresponding cell, and shall not be selected when re-allocating the PCI of the neighboring cell of the corresponding cell.";L.Polek;00-OLD-SON;
Algorithms;FGS-SF0102;PCI Auto-Configuration;FGS-SF0102 - PCI Auto-Configuration;FR350;350;"SON Server(VNF)? ??? PCI ? 5G EMS(VNF)? ???? 5G-NB ? ????? ??.
  . ?, PCI ?? ??? ??? ??? ??? ??, ??? ?? ?? ? 5G EMS(VNF)? ????.

SON Server (VNF) shall transfer the selected PCI to 5G-NB via 5G EMS (VNF).
...
Algorithms;FGS-SF0102;PCI Auto-Configuration;FGS-SF0102 - PCI Auto-Configuration;FR360;360;"5G-NB ? ??? PCI ?? ??? ?? PCI ? ????.  ??, PCI ?? ??? ?? ????? ??.
  . (??) cell ? PCI ?? ?? ?, 5G EMS(VNF) ? ???? SON Server(VNF) ?? ?? ?? ??? ????.
  . ?, ??? ??? PCI ?? ??? ?? PCI ?? ??? PCI ?? ??? SON Server? ??? ????.
  . PCI ?? ??: Locked, Shutting_Down, Shutting_Down & Forced Handover
                         ? Shutting_Down, Shutting_Down & Forced Handover: TBD
   . PCI ?? Time: YYYY-MM-DD HH:MM:SS

5G-NB shall change the PCI according to the pre-defined PCI change method. At this time, the PCI change time should be recorded together.
  . After completing the PCI change of (all) cell(s), 5G-NB shall inform the SON Server (VNF) via 5G EMS (VNF) that the change has been completed.
  . The function of PCI change according to the pre-defined PCI change mode shall be applied only when the PCI is changed by SON Server.
  . PCI change method: Locked, Shutting_Down, Shutting_Down & Forced Handover
                             ? Shutting_Down, Shutting_Down & Forced Handover: TBD
  . PCI change Time: YYYY-MM-DD HH: MM: SS";L.Polek;00-OLD-SON;
Algorithms;FGS-SF0102;PCI Auto-Configuration;FGS-SF0102 - PCI Auto-Configuration;FR380;380;"SON Server(VNF)? 5G EMS(VNF)? ???? PCI ?? ?? ??? 5G-NB? ?? ????, ?? cell ? ??  To Be PCI ? ???? ?? ?? ?? ????.
  . ????? PCI ?? ?? ?? ?, ??? ?? ?? ??? ?? ???? ?? cell? ?? 'To Be PCI'? ????.

SON Server (VNF) shall remove/delete 'To Be PCI' of the cell that SON Server (VNF) receives the notification of PCI change from and manages 'To Be PCI' for.
  . When the time exceeds the pre-definded waiting time for operator's approval, SON Server (VNF) shall remove/delete 'To Be PCI' of the cell wating operator's approval. ";L.Polek;00-OLD-SON;
Algorithms;FGS-SF0102;PCI Auto-Configuration;FGS-SF0102 - PCI Auto-Configuration;FR390;390;"SON Server(VNF)? ?? PCI ?? ??? ?? ?? ?? ??? ????.
  - PCI ?? ??, PCI ???, PCI ?? ??? ?? ?? ? ??, PCI ?? ?? ? ??

SON Server (VNF) shall record the following history for PCI conflict detection.
  - PCI conflict detection (time, related cells), PCI reallocation (time, PCI), Operator Approval (request time, approval time), time of PCI change request and completion etc.
";L.Polek;00-OLD-SON;
Algorithms;FGS-SF0102;PCI Auto-Configuration;FGS-SF0102 - PCI Auto-Configuration;FR400;400;"SON Server (VNF)? Non-Fragmented BW CC with Lowest Cell_Num? PCI? ???? ??, ?? CA-Cell? ?? Fragmented BW CC? ?? PCI? ????.

 SON Server (VNF) shall change the PCIs of all fragmented bandwidth cells included in the same CA cell to the same PCI, when changing the PCI of non-fragmented bandwidth cell with lowest Cell_Num.
";L.Polek;00-OLD-SON;
Algorithms;FGS-SF0102;PCI Auto-Configuration;FGS-SF0102 - PCI Auto-Configuration;FR410;410;"SON Server(VNF)? ????? ?? ??? ?? ?, Fragmented BW CC? PCI? ?? CA-Cell? ??? Non-Fragmented BW CC with Lowest Cell_Num? PCI? ?? ?? ??, ?? ?? ??? ??? ????? ??. [TBD]

SON Server (VNF) should provide the function to match the PCI of Fragmented BW CC and the PCI of Non-Fragmented BW CC and Lowest Cell_Num belonging to the same CA-cell periodically or at the operator's request. [TBD]";L.Polek;00-OLD-SON;
Algorithms;FGS-SF0102;PCI Auto-Configuration;FGS-SF0102 - PCI Auto-Configuration;FR510;510;"SON Server(VNF)? ???? ??? cell ? ?? ?? ???, ?? PCI ? ???? cell ? ??? ?? (Map) ?? ??? ? ??? ??.
   - Cell ?? : ??, ??? ?
   - ??, ? cell ? ? ?? (distance) ?? ? ??(i.e. CDF or PDF) ??? ?? ??? ? ??? ??.

SON Server (VNF) shall be able to display on the map the information of the cells having the same frequency and PCI as the cell selected by the operator.
...
Algorithms;FGS-SF0102;PCI Auto-Configuration;FGS-SF0102 - PCI Auto-Configuration;FR520;520;"SON Server(VNF)? ???? ??? cell? ?? ?? ???, ?? PCI ? ???? cell ? ??? ?? (Map) ?? ??? ? ??? ??.
   - ?? PCI: ???? PCI ?? ?, ??? ?? ???? ??? PCI
   - Cell ??: ??, ??? ?
   - ??, ? cell ?? ?? (distance) ?? ? ??(i.e. CDF of PDF) ??? ?? ??? ? ??? ??.

SON Server (VNF) shall be able to display on the map the information of the cells having the same frequency as the cell selected by the operator and specific PCI.
   - Information: location, azimuth angle, etc.
...
Algorithms;FGS-SF0102;PCI Auto-Configuration;FGS-SF0102 - PCI Auto-Configuration;FR530;530;" SON Server(VNF)? ???? ??? ??/???? ?? PCI?? minimum reuse distance? graph? ??? ? ??? ??.
   . ????: 5G-NB/Cell group,  ???? ?? ??

SON Server (VNF) shall be able to display the graph for the minimum reuse distance per PCI for the area / frequency selected by the operator.
   . Area: 5G-NB / Cell group, area selected on the map
";L.Polek;00-OLD-SON;
Algorithms;FGS-SF0102;PCI Auto-Configuration;FGS-SF0102 - PCI Auto-Configuration;FR540;540;"ON Server(VNF)? ???? ??? ??? ?? , ??? ???/PCI ? ???? cell ? ??? ?? (Map) ?? ??? ? ??? ??.
   . Cell ?? : ??, ??? ?

SON Server (VNF) shall be able to display on the map the information of the cells having the specific frequency and PCI for the area selected by the operator.
   . Cell information: location, azimuth, etc.";L.Polek;00-OLD-SON;
Algorithms;FGS-SF0102;PCI Auto-Configuration;FGS-SF0102 - PCI Auto-Configuration;FR550;550;"SON Server(VNF)? PCI Map ? ?? ??, ?? ? ?? ??? ????? ??.

SON Server (VNF) shall support the function of zoom-in, zoom-out and movement for the PCI map.";L.Polek;00-OLD-SON;
Algorithms;FGS-SF0102;PCI Auto-Configuration;FGS-SF0102 - PCI Auto-Configuration;FR560;560;"SON Server(VNF)? PCI ?? ??? ?? ?? ?? ? ?? ?? ??? ????? ??.

SON Server (VNF) shall provide individual approval and batch approval for PCI change.";L.Polek;00-OLD-SON;
Algorithms;FGS-SF0102;PCI Auto-Configuration;FGS-SF0102 - PCI Auto-Configuration;FR570;570;"SON Server(VNF)? PCI ?? ??? file ? ???? ??? ????? ??.

SON Server (VNF) shall the function to save PCI analysis results as a file.";L.Polek;00-OLD-SON;
Algorithms;FGS-SF0104;RACH Optimization;FGS-SF0104 - RACH Optimization;FR20;20;"SON Server(VNF)? RSI white list ?? ??? ????? ??.
  - 10 discontinuous ranges for RSI white list
  - RSI white list for each cell type (i.e. Macro, Outdoor Pico, Indoor Pico  ) [TBD]

SON Server(VNF) shall be able to configure the 'RSI white list'.
  - 10 discontinuous ranges for RSI white list
  - RSI white list for each cell type (i.e. Macro, Outdoor Pico, Indoor Pico  ) [TBD]
";G.Zajac;00-OLD-SON;
Algorithms;FGS-SF0104;RACH Optimization;FGS-SF0104 - RACH Optimization;FR30;30;"SON Server(VNF)? RSI ??? ?? ?? check ? ???? ?? Functional Switch ? Activation Schedule ?? ??? ????? ??.
  - Schedule: start time, duration, period, amount

SON Server (VNF) shall be able to set the functional switch and configure activation schedule to check RSI re-allocation necessity and re-allocate RSI.
  - Activation schedule: start time, duration, period, amount";G.Zajac;00-OLD-SON;
Algorithms;FGS-SF0104;RACH Optimization;FGS-SF0104 - RACH Optimization;FR40;40;"SON Server(VNF)? RSI ??? ?? ?? check ? ??? ? ???? ????? ???? ??? ????? ??.
  - ????: ?? ??, Random Access ?? threshold (Count, KPI ?) ? Handover ?? threshold (Count, KPI ?)

SON Server (VNF) shall be able to configure the parameters to be used in checking RSI re-allocation necessity and re-allocate RSI.
  - Parameters: Statistics Duration, Random Access related threshold (Count, KPI, etc.) and Handover related threshold (Count, KPI, etc.)";G.Zajac;00-OLD-SON;
Algorithms;FGS-SF0104;RACH Optimization;FGS-SF0104 - RACH Optimization;FR210;210;"SON Server (VNF)? ?? 3?? ?? ?? ?, Initial RSI? ???? 5G EMS (VNF)? ????? ??.
  . Cell creation complete
  ...
  . AU's GPS location (Lat./Log) information is acquired.

SON Server (VNF) shall select and transfer the initial RSI to 5G EMS (VNF), when satisfing the following conditions.
  . Cell creation complete
  . The flag in cell creation profile is set to be TRUE. The flag in cell creation profile means whether the initial RSI is allocated automatically or not
  . AU's GPS location (Lat./Log) information is acquired.";G.Zajac;00-OLD-SON;
Algorithms;FGS-SF0104;RACH Optimization;FGS-SF0104 - RACH Optimization;FR220;220;"SON Server (VNF)? Cell Creation Profile?? ???? ?? initial parameter ? ?? ?, ?? cell? Cell Creation Profile? ???? ??.
  - Initial parameters: PCI, NR, RSI, etc.
...
";G.Zajac;00-OLD-SON;
Algorithms;FGS-SF0104;RACH Optimization;FGS-SF0104 - RACH Optimization;FR230;230;"SON Server(VNF)? ?????, ???? ?? Random Access ?? ?? ?? ???? RSI ??? ?? ?? ?? ??? ????? ??.
   - ?? ? threshold ? ?? ?? ????? ? (FR40 ??)

SON Server (VNF) shall be able to check the necessity of RSI re-allocation based on the performance statistics related to random access periodically.
   - Period and thresholds shall be predefined (FR40)
";G.Zajac;00-OLD-SON;
Algorithms;FGS-SF0104;RACH Optimization;FGS-SF0104 - RACH Optimization;FR240;240;"SON Server(VNF)? RSI ??? ??? cell? ?? ?? ?? ??? handover ?? ?? ???? RSI ????? ??.
  - ?, RSI white list ??? RSI? ????? ??.

SON Server (VNF) shall select RSI based on the handover statistics for the cell that need RSI reallocation.
  - The RSI shall be selected only in the RSI white list.";G.Zajac;00-OLD-SON;
Algorithms;FGS-SF0104;RACH Optimization;FGS-SF0104 - RACH Optimization;FR250;250;"SON Server(VNF)? ??? RSI ? 5G EMS(VNF)? ???? 5G-NB ? ????? ??.

SON Server (VNF) shall transfer the selected RSI to 5G-NB via 5G EMS (VNF).";G.Zajac;00-OLD-SON;
Algorithms;FGS-SF0104;RACH Optimization;FGS-SF0104 - RACH Optimization;FR260;260;"5G-NB ? RSI ?? ?? ?? ?, ?? ?, 5G EMS(VNF)? ???? SON Server(VNF)?? ?? ?? ??? ????. ??, RSI ?? ??? ?? ????? ??.
  - RSI ?? ??: YYYY-MM-DD HH:MM:SS

When receiving RSI change request, 5G-NB shall inform the RSI change to SON Server (VNF) via 5G EMS (VNF) after RSI change. At this time, the RSI change time shall recorded together.
   - RSI change time: YYYY-MM-DD HH: MM: SS";G.Zajac;00-OLD-SON;
Algorithms;FGS-SF0104;RACH Optimization;FGS-SF0104 - RACH Optimization;FR270;270;"SON Server(VNF)? ?? RSI ???? ?? ?? ?? ??? ????.
  - RSI ??? ??? ??, RSI ???, RSI ?? ?? ? ?? ?

SON Server (VNF) shall record the following history for RSI re-allocation
  - RSI re-allocation recognition (time, related cells), RSI reallocation (time, RSI), time of RSI change request and completion etc.";G.Zajac;00-OLD-SON;
Algorithms;FGS-SF0104;RACH Optimization;FGS-SF0104 - RACH Optimization;FR410;410;"SON Server(VNF)? ???? ??? cell ? ?? ?? ???, ?? RSI ? ???? cell ? ??? ?? (Map) ?? ??? ? ??? ??.
   - Cell ?? : ??, ??? ?
   - ??, ? cell ? ? ?? (distance) ?? ? ?? (i.e. CDF or PDF) ??? ?? ??? ? ??? ??.

The SON Server (VNF) shall be able to display on the map the information of the cells having the same frequency and RSI as the cell selected by the operator.
   - information: location, azimuth angle, etc.
...
Algorithms;FGS-SF0104;RACH Optimization;FGS-SF0104 - RACH Optimization;FR420;420;"SON Server(VNF)? ???? ??? cell? ?? ?? ???, ?? RSI? ???? cell ? ??? ?? (Map) ?? ??? ? ??? ??.
   - ?? RSI:???? RSI ?? ?, ??? ?? ???? ??? RSI
   - Cell ??: ??, ??? ?
   - ??, ? cell ?? ?? (distance) ?? ? ?? (i.e. CDF or PDF) ??? ?? ??? ? ??? ??.

SON Server (VNF) shall be able to display on the map the information of the cells having the same frequency as the cell selected by the operator and specific RSI.
   - Information: location, azimuth angle, etc.
   - At this time, distance and distance distribution (i.e. CDF or PDF) from each cell to the selected cell shall be able to displayed together.
   - Background: support that operator can check RSI feasibility on the map before RSI change";G.Zajac;00-OLD-SON;
Algorithms;FGS-SF0104;RACH Optimization;FGS-SF0104 - RACH Optimization;FR430;430;"SON Server(VNF)? ???? ??? ??/???? ?? RSI?? minimum reuse distance?  graph? ??? ? ??? ??.
   - ????: 5G-NB/Cell group,  ???? ?? ??

SON Server (VNF) shall be able to display the graph for the minimum reuse distance per RSI for the area / frequency selected by the operator.
   - Area: 5G-NB / Cell group,  area selected on the map";G.Zajac;00-OLD-SON;
Algorithms;FGS-SF0104;RACH Optimization;FGS-SF0104 - RACH Optimization;FR440;440;"SON Server(VNF)? ???? ??? ??? ?? , ??? ???/RSI ? ???? cell ? ??? ?? (Map) ?? ??? ? ??? ??.
   - Cell ?? : ??, ??? ?

SON Server (VNF) shall be able to display on the map the information of the cells having the specific frequency and RSI for the area selected by the operator.
   - Cell information: location, azimuth, etc.";G.Zajac;00-OLD-SON;
Algorithms;FGS-SF0104;RACH Optimization;FGS-SF0104 - RACH Optimization;FR450;450;"SON Server(VNF)? ???? ??? ??/???? ?? cell ?? RACH ?? ?? ??? ?? (Map) ?? ??? ? ??? ??.
   - RACH ?? ??: RA Msg. 3 decoding ???, CBRA Msg.3 decoding ???, CFRA Msg.3 decoding ???
   - RACH ??? color (e.g. Red, Orange, Yellow, Green)? mapping ?? rule? ?? ????? ??.

SON Server (VNF) shall be able to display on the map RACH-related performance information for each cell for the selected area / frequency by operator.
...
Algorithms;FGS-SF0104;RACH Optimization;FGS-SF0104 - RACH Optimization;FR460;460;"SON Server(VNF)? RSI ? RACH ?? Map ? ?? ??, ?? ? ?? ??? ????? ??.

SON Server (VNF) shall support the function of zoom-in, zoom-out and movement for the RSI and RACH performance map.";G.Zajac;00-OLD-SON;
Algorithms;FGS-SF0104;RACH Optimization;FGS-SF0104 - RACH Optimization;FR470;470;"SON Server(VNF)? RSI ? RACH ?? ?? ??? file? ???? ??? ????? ??.

SON Server (VNF) shall the function to save RSI and RACH performance analysis results as a file.";G.Zajac;00-OLD-SON;
